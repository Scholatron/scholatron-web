...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log("🔄 Verifying with backend:", backendUrl);
    ...
    if (!verifyResponse.ok) {
      ...
      console.error("❌ Backend verification failed:", verifyResponse.status, errorText);
      return NextResponse.redirect(
        new URL("/auth/error?error=backend_verification_failed", request.url)
      );
    }
    ...
    if (!customJWT || !user) {
      ...
      return NextResponse.redirect(
        new URL("/auth/error?error=invalid_response", request.url)
      );
    }

    console.log("✅ Backend verification successful for user:", user.id);
